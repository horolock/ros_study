...
public:
    SimpleParameter() : Node("simple_parameter")
    {
        ...
        this->declare_parameter<std::string>("simple_string_param", "Ho");

        param_callback_handle_ = this->add_on_set_parameters_callback(std::bind(&SimpleParameter::paramChangeCallback, this, std::placeholders::_1));
    }
...
private:
    rcl_interfaces::msg::SetParametersResult paramChangeCallback(const std::vector<rclcpp::Parameter> &parameters)
    {
        ...
        for (const auto &param : parameters)
        {
            ...
            {
                RCLCPP_INFO_STREAM(this->get_logger(), "Param simple_string_param changed! New value is : " << param.as_string());
                result.successful = true;
            }
        }
